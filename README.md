...
Grab `wrt-inspector.zip` from the [latest release](https://github.com/quietcactus/what-renders-this/releases/latest), then either:

- **Upload it** — **Plugins → Add New → Upload Plugin**, pick the zip, activate.
- **Unpack it** — extract `wrt-inspector/` into `wp-content/plugins/`, activate **What Renders This** in **Plugins**.
...
There is no build step — the source in `includes/` and `assets/` is what runs. `dist/` is gitignored; it is a scratch copy of the installable files plus the zip, rebuilt at release time and attached to the GitHub Release:
